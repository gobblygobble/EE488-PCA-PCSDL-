...
__global__ void kernelRenderCircles() {
    ...
    int maxX = min(imageWidth, (blockIdx.x + 1) * blockDim.x) - 1;
    int maxY = min(imageHeight, (blockIdx.y + 1) * blockDim.y) - 1;

    int pixelX = minX + threadIdx.x;
    int pixelY = minY + threadIdx.y;

    if (pixelX > maxX || pixelY > maxY) return;
    ...
    __shared__ int indexCircle[CAPACITY];
    __shared__ int depthCircle[CAPACITY];
    __shared__ int howManyOverlapping;

    if (threadIdx.x == 0 && threadIdx.y == 0) {

        howManyOverlapping = 0;
        ...
        for (int i = 0; i < numCircles; i++) {
            ...
            if (circleInBoxConservative(p.x, p.y, radius, boxL, boxR, boxB, boxT)) {
                if (howManyOverlapping >= CAPACITY) break;

                int j;
                for (j = howManyOverlapping - 1; ((j >= 0) && (depthCircle[j] > p.z)); j--) {
                    indexCircle[j + 1] = indexCircle[j];
                    depthCircle[j + 1] = depthCircle[j];
                }
                indexCircle[j + 1] = i;
                depthCircle[j + 1] = p.z;
    ...
    for (int i = howManyOverlapping - 1; i >= 0; i--) {
        float3 p = *(float3 *)(&cuConstRendererParams.position[3 * indexCircle[i]]);
        shadePixel(indexCircle[i], pixelCenterNorm, p, imgPtr);
    }
...
/*
    ...
    if (threadIdx.x == 0 && threadIdx.y == 0) {
        for (int i = 0; i < numCircles; i++) {
            float3 p = *(float3 *)(&cuConstRendererParams.position[3 * i]);
            float radius = cuConstRendererParams.radius[i];
            if (circleInBoxConservative(p.x, p.y, radius, boxL, boxR, boxB, boxT)) A[K++] = i;
        }
    }
    __syncthreads();
