...
saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
...
#ifdef __CPU__
    // check CPU time to do the same job
    double cpuStartTime = CycleTimer::currentSeconds();
    for (int i = 0; i < N; i++) resultarray[i] = alpha * xarray[i] + yarray[i];
    ...
    printf("Kernel time: %.3f ms\t\t[%3.f GB/s]\n", 1000.f * kernelDuration, toBW(totalBytes, kernelDuration));

#ifdef __CPU__
    double cpuDuration = cpuEndTime - cpuStartTime;
